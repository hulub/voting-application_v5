...
public abstract class AbstractTrezorHardwareWalletClient implements HardwareWalletClient {
	...
	public Optional<MessageEvent> initialise() {
		return sendMessage(TrezorMessage.Initialize.newBuilder().build());
	}

	@Override
	public Optional<MessageEvent> ping() {
		return sendMessage(TrezorMessage.Ping.newBuilder().build());
	}

	@Override
	public Optional<MessageEvent> clearSession() {
		return sendMessage(TrezorMessage.ClearSession.newBuilder().build());
	}

	@Override
	public Optional<MessageEvent> changePIN(boolean remove) {

		return sendMessage(TrezorMessage.ChangePin.newBuilder().setRemove(remove).build());
	}

	@Override
	public Optional<MessageEvent> wipeDevice() {
		return sendMessage(TrezorMessage.WipeDevice.newBuilder().build());
	}
	...
	public Optional<MessageEvent> getEntropy() {
		return sendMessage(TrezorMessage.GetEntropy.newBuilder().build());
	}

	// TODO Support use of seed node
	@Override
	public Optional<MessageEvent> loadDevice(String language, String label, String seedPhrase, String pin) {

		// A load normally takes about 10 seconds to complete
		return sendMessage(
				TrezorMessage.LoadDevice.newBuilder().setLanguage(language).setLabel(label).setMnemonic(seedPhrase)
						// .setNode(nodeType)
						.setPin(pin).build());
	...
	@Override
	public Optional<MessageEvent> resetDevice(String language, String label, boolean displayRandom,
			boolean pinProtection, int strength) {

		return sendMessage(TrezorMessage.ResetDevice.newBuilder().setLanguage(language).setLabel(label)
				.setDisplayRandom(displayRandom).setStrength(strength).setPinProtection(pinProtection).build());

	}

	@Override
	public Optional<MessageEvent> recoverDevice(String language, String label, int wordCount,
			boolean passphraseProtection, boolean pinProtection) {
		return sendMessage(
				TrezorMessage.RecoveryDevice.newBuilder().setLanguage(language).setLabel(label).setWordCount(wordCount)
						.setPassphraseProtection(passphraseProtection).setPinProtection(pinProtection).build());
	}

	@Override
	public Optional<MessageEvent> wordAck(String word) {
		return sendMessage(TrezorMessage.WordAck.newBuilder().setWord(word).build());
	}

	@Override
	public Optional<MessageEvent> signTx(Transaction tx) {

		return sendMessage(TrezorMessage.SignTx.newBuilder().setCoinName("Bitcoin")
				.setInputsCount(tx.getInputs().size()).setOutputsCount(tx.getOutputs().size()).build());
	...
	public Optional<MessageEvent> simpleSignTx(Transaction tx) {
		...
		for (TransactionInput input : tx.getInputs()) {
			...
			TrezorType.TxInputType txInputType = TrezorType.TxInputType.newBuilder()
					.setSequence((int) input.getSequenceNumber())
					.setScriptSig(ByteString.copyFrom(input.getScriptSig().getProgram())).setScriptType(inputScriptType)
					.setPrevIndex(prevIndex).setPrevHash(ByteString.copyFrom(prevHash)).build();
		...
		for (TransactionOutput output : tx.getOutputs()) {
			...
			TrezorType.TxOutputType txOutputType = TrezorType.TxOutputType.newBuilder()
					.setAddress(String.valueOf(address)).setAmount(output.getValue().value)
					.setScriptType(outputScriptType).build();
		...
		for (TransactionInput input : tx.getInputs()) {
			...
			for (TransactionInput prevInput : prevTx.getInputs()) {
				...
				// No multisig support in MBHD yet so all input scripts will be
				// to addresses (P2PKH)
				TrezorType.InputScriptType inputScriptType = TrezorType.InputScriptType.SPENDADDRESS;
				...
				TrezorType.TxInputType txInputType = TrezorType.TxInputType.newBuilder()
						...
						.setScriptType(inputScriptType).setPrevIndex(prevIndex)
			...
			for (TransactionOutput prevOutput : prevTx.getOutputs()) {
				...
				TrezorType.TxOutputBinType txOutputBinType = TrezorType.TxOutputBinType.newBuilder()
						.setAmount(prevOutput.getValue().value)
						.setScriptPubkey(ByteString.copyFrom(prevOutput.getScriptPubKey().getProgram())).build();
	...
	public Optional<MessageEvent> txAck(TxRequest txRequest, Transaction tx,
		...
		// Assume we're working with the current (child) transaction to start
		// with
		...
		case TX_META:
			...
			break;
		case TX_INPUT:
			txType = TrezorMessageUtils.buildTxInputResponse(txRequest, requestedTx, binOutputType,
		...
		if (txType != null) {
			return sendMessage(TrezorMessage.TxAck.newBuilder().setTx(txType).build());
		}
	...
	public Optional<MessageEvent> pinMatrixAck(String pin) {
		return sendMessage(TrezorMessage.PinMatrixAck.newBuilder().setPin(pin).build(),
	...
	public Optional<MessageEvent> passphraseAck(String passphrase) {
		return sendMessage(TrezorMessage.PassphraseAck.newBuilder().setPassphrase(passphrase).build(),
				// No immediate response expected
				2, TimeUnit.SECONDS);
	}
	...
	public Optional<MessageEvent> buttonAck() {
		return sendMessage(TrezorMessage.ButtonAck.newBuilder().build(),
				// No immediate response expected
				2, TimeUnit.SECONDS);
	}

	public Optional<MessageEvent> cancel() {
		return sendMessage(TrezorMessage.Cancel.newBuilder().build(),
				// No immediate response expected
				2, TimeUnit.SECONDS);
	}

	@Override
	public Optional<MessageEvent> getAddress(int account, KeyChain.KeyPurpose keyPurpose, int index,
			boolean showDisplay) {

		return sendMessage(TrezorMessage.GetAddress.newBuilder()
				// Build the chain code
				.addAllAddressN(TrezorMessageUtils.buildAddressN(account, keyPurpose, index)).setCoinName("Bitcoin")
				.setShowDisplay(showDisplay).build());
	}

	@Override
	public Optional<MessageEvent> getPublicKey(int account, KeyChain.KeyPurpose keyPurpose, int index) {

		return sendMessage(TrezorMessage.GetPublicKey.newBuilder()
				// Build the chain code
				.addAllAddressN(TrezorMessageUtils.buildAddressN(account, keyPurpose, index)).build());

	}

	@Override
	public Optional<MessageEvent> getPublicKeyForIdentity(URI identityUri, int index, String ecdsaCurveName,
			boolean showDisplay) {

		return sendMessage(TrezorMessage.GetPublicKey.newBuilder()
				// Build the chain code
				.addAllAddressN(IdentityUtils.buildAddressN(identityUri, index)).setEcdsaCurveName(ecdsaCurveName)
				.setShowDisplay(showDisplay).build());
	}
	...
	public Optional<MessageEvent> getDeterministicHierarchy(List<ChildNumber> childNumbers) {
		...
		return sendMessage(TrezorMessage.GetPublicKey.newBuilder().addAllAddressN(addressN).build());
	...
	public Optional<MessageEvent> applySettings(String language, String label) {
		...
		return sendMessage(TrezorMessage.ApplySettings.newBuilder().setLanguage(language).setLabel(label).build());
	}

	@Override
	public Optional<MessageEvent> entropyAck(byte[] entropy) {
		return sendMessage(TrezorMessage.EntropyAck.newBuilder().setEntropy(ByteString.copyFrom(entropy)).build());
	}

	@Override
	public Optional<MessageEvent> signMessage(int account, KeyChain.KeyPurpose keyPurpose, int index, byte[] message) {
		return sendMessage(TrezorMessage.SignMessage.newBuilder()
				// Build the chain code
				.addAllAddressN(TrezorMessageUtils.buildAddressN(account, keyPurpose, index)).setCoinName("Bitcoin")
	...
	/* Ring Sign Message */
	@Override
	public Optional<MessageEvent> ringSignMessage(List<byte[]> L, int n, int pi, byte[] message) {

		TrezorMessage.RingSignMessage.Builder builder = TrezorMessage.RingSignMessage.newBuilder();

		// add the public key list
		for (byte[] pk : L) {
			// add one public key at a time
			builder.addL(com.google.protobuf.ByteString.copyFrom(pk));
		}
	...
	/* Get Public Key 65 */
	@Override
	public Optional<MessageEvent> getPublicKey65() {
		
		TrezorMessage.GetPublicKey65.Builder pkBuilder = TrezorMessage.GetPublicKey65.newBuilder();

		return sendMessage(pkBuilder.build());
	}

	@Override
	public Optional<MessageEvent> verifyMessage(Address address, byte[] signature, byte[] message) {
		return sendMessage(TrezorMessage.VerifyMessage.newBuilder().setAddress(address.toString())
				.setSignature(ByteString.copyFrom(signature)).setMessage(ByteString.copyFrom(message)).build());
	}

	@Override
	public Optional<MessageEvent> encryptMessage(byte[] pubKey, byte[] message, boolean displayOnly) {
		return sendMessage(TrezorMessage.EncryptMessage.newBuilder().setPubkey(ByteString.copyFrom(pubKey))
				.setMessage(ByteString.copyFrom(message)).setDisplayOnly(displayOnly).build());
	}

	@Override
	public Optional<MessageEvent> decryptMessage(int account, KeyChain.KeyPurpose keyPurpose, int index,
			byte[] message) {
		return sendMessage(TrezorMessage.DecryptMessage.newBuilder()
				// Build the chain code
	...
	@Override
	public Optional<MessageEvent> cipherKeyValue(int account, KeyChain.KeyPurpose keyPurpose, int index, byte[] key,
			byte[] keyValue, boolean isEncrypting, boolean askOnDecrypt, boolean askOnEncrypt) {
		return sendMessage(TrezorMessage.CipherKeyValue.newBuilder()
				// Build the chain code
				.addAllAddressN(TrezorMessageUtils.buildAddressN(account, keyPurpose, index))
				.setAskOnDecrypt(askOnDecrypt).setAskOnEncrypt(askOnEncrypt).setEncrypt(isEncrypting)
				.setKeyBytes(ByteString.copyFrom(key)).setValue(ByteString.copyFrom(keyValue)).build());
	}
	...
	public Optional<MessageEvent> estimateTxSize(Transaction tx) {
		...
		return sendMessage(TrezorMessage.EstimateTxSize.newBuilder().setCoinName("Bitcoin").setInputsCount(inputsCount)
				.setOutputsCount(outputsCount).build());
	}
	...
	public Optional<MessageEvent> signIdentity(Identity identity) {
		...
		TrezorType.IdentityType identityType = TrezorType.IdentityType.newBuilder().setHost(identity.getHost())
				.setPath(identity.getPath()).setPort(identity.getPort()).setProto(identity.getProto())
				.setUser(identity.getUser()).setIndex(identity.getIndex()).build();

		return sendMessage(TrezorMessage.SignIdentity.newBuilder()
				.setChallengeHidden(ByteString.copyFrom(identity.getChallengeHidden()))
				.setChallengeVisual(identity.getChallengeVisual()).setEcdsaCurveName(identity.getEcdsaCurveName())
				.setIdentity(identityType).build());
	}
	 ...
	 *
	 * @param message
	 *            The message to send to the hardware wallet
	 *
	 ...
	 *            The message to send to the hardware wallet
	 * @param duration
	 *            The duration to wait before returning
	 * @param timeUnit
	 *            The time unit
	 *
	 * @return An optional low level message event, present only in blocking
